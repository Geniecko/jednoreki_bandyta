class Game{
	...
	render(colors = ['gray', 'gray', 'gray'],money = this.wallet.getWalletValue(),result = "" ,stats = [0,0,0], bid = 0, wonMoney = 0){
		...
		this.boards.forEach(function(item,i){
	...
	startGame(){
		...
		if(!this.wallet.checkCanPlay(bid)) {
			return alert("Masz za mało srodków lub została podana nie prawidłowa wartość")
		}
		...
		const win = Results.checkWinner(colors);
		const wonMoney = Results.moneyWinInGame(win, bid);
